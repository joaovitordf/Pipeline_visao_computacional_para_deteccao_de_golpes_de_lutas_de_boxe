...
HEAD_ROI_FACTOR = 0.025          # 2,5% da largura/altura para a cabeça
...
def roi_cabeca(imagem, keypoints_numpy):
    ...
    if x != 0 and y != 0:
        offset_x = int(imagem.shape[1] * HEAD_ROI_FACTOR)
        offset_y = int(imagem.shape[0] * HEAD_ROI_FACTOR)
...
def roi_mao_esquerda(imagem, keypoints_numpy):
    ...
    if x != 0 and y != 0:
        offset_x = int(imagem.shape[1] * HAND_ROI_WIDTH_FACTOR)
        offset_y = int(imagem.shape[0] * HAND_ROI_HEIGHT_FACTOR)
        start_point = (max(x - offset_x, 0), max(y - offset_y, 0))
        end_point = (min(x + offset_x, imagem.shape[1]), min(y + offset_y, imagem.shape[0]))
        return (start_point, end_point)
    else:
...
def roi_linha_cintura(imagem, keypoints_numpy):
    ...
    if (x1, y1) != (0, 0) and (x2, y2) != (0, 0):
        offset = int(imagem.shape[1] * WAIST_ROI_WIDTH_FACTOR)
...
def roi_tronco(imagem, keypoints_numpy):
    ...
    if (x1, y1) != (0, 0) and (x2, y2) != (0, 0):
        aux_min_x = min(x1, x2)
        aux_max_x = max(x1, x2)
        # Garante uma largura mínima proporcional à imagem
        min_width = int(imagem.shape[1] * MIN_TRUNK_WIDTH_FACTOR)
